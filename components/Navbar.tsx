...
export default function Navbar() {
	...
	return (
		<header className="fixed inset-x-0 top-0 z-20 px-4 pt-4 font-nian sm:px-6 sm:pt-6 lg:px-24">
			<div
				className="mx-auto flex w-full items-center justify-between gap-4 rounded-full bg-[#fcfaf4]/45 px-4 py-1 text-[#2a3834] shadow-[0_10px_30px_rgba(17,24,39,0.08)] backdrop-blur md:px-6 lg:px-8"
			// style={{ maxWidth: "1380px" }}
			>
				<div className="hidden flex-1 flex-row-reverse items-center justify-center gap-10 text-[1.25rem] font-medium tracking-[0.22em] text-[#202e29] uppercase lg:flex">
					{items.map((item) => {
				...
				<div className="flex shrink-0 items-center gap-2 sm:gap-3">
					<Link
						href="/contact"
						className="hidden items-center gap-3 rounded-full bg-[#35564d] px-4 py-2 text-sm font-medium text-[#d8efe4] transition hover:bg-[#376b5d] sm:inline-flex sm:pr-5 sm:pl-2"
					>
						<span className="text-xl tracking-widest">ارتباط با دکتر</span>
						<span className="flex h-8 w-8 items-center justify-center rounded-full bg-[#d8efe4] text-white">
							<ArrowIcon className="rotate-180 text-[#35564d]" />
						</span>
					</Link>
					<button
						type="button"
			...
			<AnimatePresence>
				{mobileOpen && <motion.nav initial={{ opacity: 0, y: -12, height: 0 }} animate={{ opacity: 1, y: 0, height: "auto" }} exit={{ opacity: 0, y: -10, height: 0 }} transition={{ duration: .3, ease: [0.22, 1, 0.36, 1] }} className="mx-auto mt-3 max-h-[calc(100vh-7rem)] overflow-y-auto rounded-[2rem] border border-[#dbe6e0] bg-[#fcfaf4] p-4 text-[#49645d] shadow-[0_22px_60px_rgba(20,52,43,.16)] lg:hidden">
					{items.map(item => item.kind === "services" ? <div key={item.id} className="border-b border-[#dce5e1]">
						<button type="button" onClick={() => setMobileServicesOpen(value => !value)} className="flex w-full items-center justify-between py-4 text-lg">
							<span>
								{item.label}</span><motion.span animate={{ rotate: mobileServicesOpen ? 180 : 0 }}><ChevronDownIcon /></motion.span></button>
						<AnimatePresence initial={false}>
							{mobileServicesOpen && <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
								<div className="space-y-1 pb-4 pr-3">
									{services.map(service =>
										<Link key={service.id} href={`/services/${service.slug}`}
											onClick={() => setMobileOpen(false)}
											className="block rounded-xl px-3 py-2.5 text-base text-[#60766f] hover:bg-[#e5f2ec]"
										>
											{service.title}
										</Link>
									)
						...
						<a key={item.id} href={item.href} onClick={() => setMobileOpen(false)}
							className="block border-b border-[#dce5e1] py-4 text-lg last:border-0"
						>
							{item.label}
					...
					<Link href="/contact" onClick={() => setMobileOpen(false)} className="mt-4 flex items-center justify-between rounded-full bg-[#d8efe4] px-5 py-3 font-medium">
						<span>ارتباط با دکتر</span>
						<span className="grid h-9 w-9 place-items-center rounded-full bg-[#35564d] text-white">
